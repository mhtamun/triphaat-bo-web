...
export const ActivityList = (tripId: string) => (
                ...
                body: (item: any) => (
                    <div
                        dangerouslySetInnerHTML={{
                            __html: !item.body ? '' : !isJSONString(item.body) ? item.body : JSON.parse(item.body),
                        }}
                    ></div>
                ),
...
const Page = ({ tripId, trip }: { tripId: string; trip: any }) => {
    ...
    return (
        <WrapperComponent tripId={tripId} title={trip?.name} router={router}>
            <TabViewComponent router={router} tripId={tripId} content={useMemo(() => ActivityList(tripId), [trip])} />
        </WrapperComponent>
    );
};
