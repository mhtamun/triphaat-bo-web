...
const TabViewComponent = ({
    ...
    const onTabChange = useCallback((e: TabViewTabChangeEvent, router: NextRouter, tripId: string) => {
        if (e.index === 0) router.push(`/v-p/fixed-date-trips/${tripId}`);
        if (e.index === 1) router.push(`/v-p/fixed-date-trips/${tripId}/variants`);
        if (e.index === 2) router.push(`/v-p/fixed-date-trips/${tripId}/images`);
        if (e.index === 3) router.push(`/v-p/fixed-date-trips/${tripId}/videos`);
        if (e.index === 4) router.push(`/v-p/fixed-date-trips/${tripId}/tags`);
        if (e.index === 5) router.push(`/v-p/fixed-date-trips/${tripId}/highlights`);
        if (e.index === 6) router.push(`/v-p/fixed-date-trips/${tripId}/includes`);
        if (e.index === 7) router.push(`/v-p/fixed-date-trips/${tripId}/faqs`);
        if (e.index === 8) router.push(`/v-p/fixed-date-trips/${tripId}/travelers`);
        if (e.index === 9) router.push(`/v-p/fixed-date-trips/${tripId}/payments`);
    }, []);
